...
def test_orchestrator_node_raises_when_policy_rejects_decision_in_debug_mode(monkeypatch):
    calls = []
    ...
    with pytest.raises(OrchestratorError, match="routed to response_agent"):
        orchestrator_node(
            {
                "task_status": {"flight": "pending"},
            }
...
def test_orchestrator_node_falls_back_when_policy_rejects_decision_in_production_mode(
    monkeypatch,
):
    def llm_decision(state, orchestration_steps):
        return OrchestratorDecision(
